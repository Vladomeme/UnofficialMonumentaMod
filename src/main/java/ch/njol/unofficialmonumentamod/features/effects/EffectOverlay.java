...
public class EffectOverlay extends HudElement {
	...
	protected void render(DrawContext context, float tickDelta) {
		...
		context.fill(0, 0, width, height, client.options.getTextBackgroundColor(UnofficialMonumentaModClient.options.overlay_opacity));
		...
		for (Effect effect : visibleEffects) {
			...
			context.drawTextWithShadow(client.textRenderer, text, textAlightRight ? width - PADDING_HORIZONTAL - textRenderer.getWidth(text) : PADDING_HORIZONTAL, currentY, 0xFFFFFFFF);
